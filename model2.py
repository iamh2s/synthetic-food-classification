...
X = label_encoder.fit_transform(food_all_data_imputed['Food_Name'])
X = X.reshape(-1, 1)
...
any_model = models["Random Forest"]    
test_food = "Chicken Curry"

# encode food name
test_encoded = label_encoder.transform([test_food])
test_encoded = test_encoded.reshape(-1, 1)

# predict output features
predicted_output = any_model.predict(test_encoded)

# convert prediction to dataframe
predicted_df = pd.DataFrame(predicted_output, columns=y.columns)
print("Predicted Features for:", test_food)
print(predicted_df)
